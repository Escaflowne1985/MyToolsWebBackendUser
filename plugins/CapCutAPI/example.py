...
import sys
import time
from plugins.CapCutAPI.util import timing_decorator
import functools
import threading
import aiohttp
import asyncio
...
async def make_request_async(endpoint, data, method='POST'):
    """Send async HTTP request to the server and handle the response"""
    url = f"{BASE_URL}/{endpoint}"
    headers = {'Content-Type': 'application/json'}
...
def test_effect_01():
    ...
    draft_folder = "D:/MyToolsData/JianYing/draft/JianyingPro Drafts"
...
async def save_draft_impl_async(draft_id, draft_folder):
    import asyncio
    """Async API wrapper for save_draft service"""
    data = {
        "license_key": LICENSE_KEY,  # Using trial version license key
        "draft_id": draft_id,
        "draft_folder": draft_folder
    }
    # 在后台线程运行同步的 make_request
    return await make_request_async("save_draft", data)
...
def test_speech_01():
    ...
    speech_result = generate_speech_impl(
        texts=["Hello everyone, welcome to my video", "Today we will discuss an interesting topic", "What to do when your child doesn't want to go to school", "Hope you enjoy this content", "Hello everyone, welcome to my video", "Today we will discuss an interesting topic",
               "What to do when your child doesn't want to go to school", "Hope you enjoy this content", "Hello everyone, welcome to my video", "Today we will discuss an interesting topic", "What to do when your child doesn't want to go to school", "Hope you enjoy this content"],
        language="Chinese",
        draft_id="123",
        speaker_id="渊博小叔",
